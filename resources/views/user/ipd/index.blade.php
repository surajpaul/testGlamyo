	...
	<div class="page-header">
	    <div class="row">
	    	...
	    	<div class="col-md-6 col-12 col-xs-12 col-sm-12" align="right">
	    		<a class="btn btn-success text-white" href="{{route('user.ipd.create')}}">Add New IPD</a>
	    	</div>
	    </div>
	</div>
	@if($ipds->count())
	        		...
	        		<td class="text-center">
	                    <div class="dropdown">
						  ...
						  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
						  	@if($ipd->status == NULL)
				            <form method="post" action="{{ route('user.ipd.complete', $ipd->id) }}">
				                @csrf
				                @method('POST')
				            ...
				            @elseif($ipd->status == 1)
				            <form method="post" action="{{ route('user.ipd.active', $ipd->id) }}">
				                @csrf
				                @method('POST')
				            ...
				            @endif
				            <a class="dropdown-item" href="{{ route('user.ipd.show', $ipd->id)}}">
						    	<i class="icon mr-2" data-feather="edit"></i> View
						    </a>
						    <a class="dropdown-item" href="{{ route('user.ipd.edit', $ipd->id)}}">
						    	<i class="icon mr-2" data-feather="edit"></i> Edit IPD
						    </a>
					    	<form action="{{ route('user.ipd.destroy', $ipd->id)}}" method="post">
			                  @csrf
			                  @method('DELETE')
			                  <button class="dropdown-item" type="submit">
